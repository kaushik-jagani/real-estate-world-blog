...
(function () {
  ...
  function applyMeta(post) {
    ...
    canonical.href = post.url || (window.location.origin + '/blog/' + post.slug);
  }
  ...
  function renderAuthorBar(post) {
    ...
    if (nameEl)   nameEl.textContent   = post.author || 'Real Estate World Team';
  ...
  function renderHero(post) {
    ...
    if (heroEl) {
      heroEl.src = toAbsolutePath(post.coverImage) || 'https://source.unsplash.com/1200x500/?dubai,real-estate';
    ...
    var cards = picks.map(function (p) {
      ...
      return '<a href="/blog/' + p.slug + '" class="post-card">' +
        ...
        '<div class="post-card-body">' +
          '<span class="badge">' + (p.category || 'Real Estate') + '</span>' +
  ...
  function init() {
    var slug = getSlug();
    if (!slug) {
      document.title = 'Post Not Found — Real Estate World';
